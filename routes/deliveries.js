  ...
  async (req, res, next) => {
    try {
      const search = new SearchBuilder(req.query)
        ...
        // Sıralama
        .addOrder(req.query.sort_by || 'delivery_date', req.query.sort_direction || 'DESC')
        // Sayfalama
  ...
  try {
    ...
    const request = await Request.findByPk(request_id, {
      include: [
        {
          model: Material,
          attributes: ['id', 'materialCode', 'description', 'unit', 'stockQty']
        },
        {
          model: Project,
...
// Teslimatları ara
router.get('/search', authenticate, cacheRoute(300), async (req, res) => {
  try {
...
// Arama önerileri al
router.get('/suggestions', authenticate, cacheRoute(300), async (req, res) => {
  try {
...
router.get('/popular-searches', authenticate, cacheRoute(3600), async (req, res) => {
  try {
