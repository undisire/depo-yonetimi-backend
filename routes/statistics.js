...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const movements = await statisticsService.getStockMovementStats(
        startDate,
        endDate
      );
...
router.get(
  "/project-material-usage/:project_id",
  auth("admin", "muhendis"),
  activityLogger("view_project_material_usage"),
  async (req, res, next) => {
    try {
      const usage = await statisticsService.getProjectMaterialUsage(
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const distribution = await statisticsService.getRequestStatusDistribution(
        startDate,
        endDate
      );
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const materials = await statisticsService.getMostUsedMaterials(
        parseInt(limit) || 10
      );
...
router.get(
  "/project-request-distribution",
  auth("admin", "muhendis"),
  activityLogger("view_project_request_distribution"),
  async (req, res, next) => {
    try {
      const distribution =
        await statisticsService.getProjectRequestDistribution();
      res.json({ data: distribution });
    } catch (error) {
      next(error);
    }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const performance = await statisticsService.getDeliveryPerformance(
        startDate,
        endDate
      );
